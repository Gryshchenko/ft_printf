...
/*   Updated: 2018/02/26 13:16:53 by vgryshch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int						print_hexadecimal_signed(t_flags *flags, va_list ap)
{
	...
	hex_arg_to_str_signed(&str, c);
	c_copy = c;
	print_precision(flags, &str);
	flag_plus(flags, &str, c);
	if (flags->minus)
		flag_hash_hexadecimal(flags, &str, c);
...
int						print_hexadecimal(va_list ap, t_flags *flags)
{
	...
	len = 0;
	if (!flags->length)
		len = print_hexadecimal_int(flags, ap);
	if (flags->length == 3 || flags->length == 6 || flags->length == 4 ||
			flags->length == 7)
		len = print_hexadecimal_long(flags, ap);
	if (flags->length == 2)
		len = print_hexadecimal_signed(flags, ap);
	if (flags->length == 1)
		len = print_hexadecimal_short(flags, ap);
	return (len);
}
